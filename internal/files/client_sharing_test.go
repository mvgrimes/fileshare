...
func (s *clientAuthorizerStub) AuthorizeClientUpload(
	_ context.Context,
	p auth.Principal,
...
func TestClientSharingServiceShareSuccess(t *testing.T) {
	...
	repo := &clientShareRepoStub{
		file: db.File{ID: "f-client", UploaderType: "client", UploaderID: "client-1"},
	}
	...
	if repo.createdShare.TargetType != "user_group" || repo.createdShare.TargetID != "ug-1" {
		t.Fatalf(
			"target = (%q,%q), want (user_group,ug-1)",
			repo.createdShare.TargetType,
			repo.createdShare.TargetID,
		)
	}
	if !repo.createdShare.Message.Valid ||
		repo.createdShare.Message.String != "quarterly documents" {
...
func TestClientSharingServiceShareValidationAndAuthorization(t *testing.T) {
	...
	t.Run("invalid target type", func(t *testing.T) {
		...
		_, err := svc.Share(
			context.Background(),
			ClientShareInput{Actor: principal, FileID: "f1", TargetType: "client", TargetID: "c1"},
		)
	...
	t.Run("authz denied", func(t *testing.T) {
		...
		_, err := svc.Share(
			context.Background(),
			ClientShareInput{Actor: principal, FileID: "f1", TargetType: "user", TargetID: "u1"},
		)
...
func TestClientSharingServiceFileShareOwnershipAndErrors(t *testing.T) {
	...
	t.Run("missing file behaves as forbidden", func(t *testing.T) {
		svc := NewClientSharingService(
			&clientAuthorizerStub{},
			&clientShareRepoStub{getFileErr: sql.ErrNoRows},
		)
		_, err := svc.Share(
			context.Background(),
			ClientShareInput{
	...
	t.Run("ownership mismatch", func(t *testing.T) {
		repo := &clientShareRepoStub{
			file: db.File{ID: "f-other", UploaderType: "client", UploaderID: "client-2"},
		}

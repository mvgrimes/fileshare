...
- Set a local SQLite `FILESHARE_DATABASE_URL` (for example: `FILESHARE_DATABASE_URL=fileshare.db`).
- Run:
  - `go run . migrate up`
...
- Use a writable SQLite path in the job workspace (for example: `FILESHARE_DATABASE_URL=$PWD/fileshare-ci.db`).
- Run migrations in order to validate both apply and rollback paths:
  - `go run . migrate up`
  - `go run . migrate status`
  - `go run . migrate down`

## FILESHARE_DATABASE_URL Rules
